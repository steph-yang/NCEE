...
```{r tidy=TRUE}
setwd("C:/Users/un150/Downloads");library(ggplot2);library(gridExtra); library(tidyverse);library(wesanderson);library(maps);library(mapdata);library(sp)
...
geom_bar(stat="identity", color="grey", position=position_dodge()) +
  ...
  ggtitle("National Data from 1987 to 2000") +
...
geom_bar(stat="identity", color="grey", position=position_dodge()) +
  ...
  ggtitle("Beijing Data from 1987 to 2000") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Year of Birth")
...
geom_bar(stat="identity", color="grey", position=position_dodge()) +
  ...
  ggtitle("Henan Data from 1987 to 2000") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Year of Birth")
...
geom_bar(stat="identity", color="grey", position=position_dodge()) +
  ...
  ggtitle("Xinjiang Data from 1987 to 2000") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("Year of Birth")
...
ggplot(data=plot_df, aes(x=year_birth, y= population, fill=label)) +
  ...
  geom_ribbon(aes(ymin=predict_national_pl, ymax=predict_national_ph,x=year_birth), linetype=2, alpha=0.4) +
    theme_minimal() +
    scale_fill_brewer(palette="Blues") +
    ggtitle("National Prediction from 1987 to 2000") +
    theme(plot.title = element_text(hjust = 0.5)) +
    xlab("Year of Birth")
...
year_prd <- c(rep("2023",31),rep("2022",31),rep("2021",31),rep("2020",31),rep("2019",31))
...
ggplot(aes(year_prd, city.fit.nu, color=province)) + 
  ...
  ggtitle("3 Metropolis & Hainan Island") +
  theme(plot.title = element_text(hjust = 0.5))

pl2 <-
pro_bind %>% subset(province %in% c("Qinghai", "Ningxia","Xizang")) %>%
ggplot(aes(year_prd, city.fit.nu, color=province)) + 
  geom_pointrange(aes(ymin = city.low.nu, ymax = city.high.nu)) +
...
pl7 <-
pro_bind %>% subset(province %in% c("Hunan", "Hubei","Anhui","Henan","Shandong")) %>%
ggplot(aes(year_prd, city.fit.nu, color=province)) + 
  geom_pointrange(aes(ymin = city.low.nu, ymax = city.high.nu)) +
      scale_color_manual(values=wes_palette("Cavalcanti1",5)) +
  geom_line(aes(group = province)) +
  theme_minimal() +
  xlab("Year") +
  ylab("Number of Reg Prediction") +
  ggtitle("North-Central 5 provinces") +
  theme(plot.title = element_text(hjust = 0.5))
